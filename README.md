# 每日 GitHub 动态 (2026-09-25)

我关注用户的今日公开动态 (每60分钟更新)。

## 今日动态

### [0xbinder](https://github.com/0xbinder)
- 🌟 👤 [0xbinder](https://github.com/0xbinder) Starred [androguard/androguard](https://github.com/androguard/androguard)
  > Reverse engineering and pentesting for Android applications 
- 🌟 👤 [0xbinder](https://github.com/0xbinder) Starred [ev-flow/quark-engine](https://github.com/ev-flow/quark-engine)

### [0xdea](https://github.com/0xdea)
- 🌟 👤 [0xdea](https://github.com/0xdea) Starred [tantosec/tturl](https://github.com/tantosec/tturl)
  > The HTTP/2 CLI for finding tiny timing differences and winning request races

### [ChiChou](https://github.com/ChiChou)
- 🍴 👤 [ChiChou](https://github.com/ChiChou) Forked [tobi/disktree](https://github.com/tobi/disktree) to [ChiChou/disktree](https://github.com/ChiChou/disktree)
  > A treemap for finding and removing what fills your disk, for Omarchy. Rust + GPUI.

### [Haleclipse](https://github.com/Haleclipse)
- 🌟 👤 [Haleclipse](https://github.com/Haleclipse) Starred [padovanl/portop](https://github.com/padovanl/portop)
  > 🔌 What's really using your ports? htop-style TUI, one keypress to kill it.

### [J-jaeyoung](https://github.com/J-jaeyoung)
- 🌟 👤 [J-jaeyoung](https://github.com/J-jaeyoung) Starred [wiz-sec-public/zeroday-cloud-2026](https://github.com/wiz-sec-public/zeroday-cloud-2026)

### [LLeavesG](https://github.com/LLeavesG)
- 🌟 👤 [LLeavesG](https://github.com/LLeavesG) Starred [Recurse-Labs/recurse](https://github.com/Recurse-Labs/recurse)
  > AI Native IDE for reverse engineering

### [divyam234](https://github.com/divyam234)
- 🌟 👤 [divyam234](https://github.com/divyam234) Starred [Spotifyd/spotifyd](https://github.com/Spotifyd/spotifyd)
  > A spotify daemon

### [ergrelet](https://github.com/ergrelet)
- 🌟 👤 [ergrelet](https://github.com/ergrelet) Starred [valeria-org/bitwright](https://github.com/valeria-org/bitwright)
  > Bit-vector expression simplifier for binary analysis and deobfuscation
- 🌟 👤 [ergrelet](https://github.com/ergrelet) Starred [YuroGod/VMPStaticUnpacker](https://github.com/YuroGod/VMPStaticUnpacker)
  > Static VMProtect unpacker , supporting VMProtect 3.9.5

### [jonpalmisc](https://github.com/jonpalmisc)
- 🌟 👤 [jonpalmisc](https://github.com/jonpalmisc) Starred [m4b/goblin](https://github.com/m4b/goblin)
  > An impish, cross-platform binary parsing crate, written in Rust

### [mrexodia](https://github.com/mrexodia)
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [ngwg/ceasta](https://github.com/ngwg/ceasta)
  > disassembler, decompiler and debugger in one, with a built-in mcp server — point an ai at a binary a...
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [valeria-org/bitwright](https://github.com/valeria-org/bitwright)
  > Bit-vector expression simplifier for binary analysis and deobfuscation
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [zhaoxuya520/reverse-skill](https://github.com/zhaoxuya520/reverse-skill)
  > Reverse Engineering / Authorized Penetration Testing / Security Research Skill Router Pack AI-powere...
- 🍴 👤 [mrexodia](https://github.com/mrexodia) Forked [jtsylve/re-mcp](https://github.com/jtsylve/re-mcp) to [mrexodia/re-mcp](https://github.com/mrexodia/re-mcp)
  > A headless MCP server for IDA Pro and Ghidra

### [osm0sis](https://github.com/osm0sis)
- 🌟 👤 [osm0sis](https://github.com/osm0sis) Starred [capntrips/PixelPlucker](https://github.com/capntrips/PixelPlucker)
  > Pixel Plucker is a cli utility that surgically extracts partitions and files out of remote Pixel fac...

### [phodal](https://github.com/phodal)
- 🌟 👤 [phodal](https://github.com/phodal) Starred [deepseek-ai/3FS](https://github.com/deepseek-ai/3FS)
  >  A high-performance distributed file system designed to address the challenges of AI training and in...


---
*最后更新于 2026-09-25 16:26:02 UTC*
*历史记录保存在 `archive` 目录中。*
