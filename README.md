# 每日 GitHub 动态 (2026-03-10)

我关注用户的今日公开动态 (每60分钟更新)。

## 今日动态

### [LLeavesG](https://github.com/LLeavesG)
- 🌟 👤 [LLeavesG](https://github.com/LLeavesG) Starred [HKUDS/CLI-Anything](https://github.com/HKUDS/CLI-Anything)
  > CLI-Anything: Making ALL Software Agent-Native

### [MouriNaruto](https://github.com/MouriNaruto)
- 🌟 👤 [MouriNaruto](https://github.com/MouriNaruto) Starred [unslothai/unsloth](https://github.com/unslothai/unsloth)
  > Fine-tuning & Reinforcement Learning for LLMs. 🦥 Train OpenAI gpt-oss, DeepSeek, Qwen, Llama, Gemma,...

### [gatieme](https://github.com/gatieme)
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [anthropics/claude-code](https://github.com/anthropics/claude-code)
  > Claude Code is an agentic coding tool that lives in your terminal, understands your codebase, and he...
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [ahmadawais/chartli](https://github.com/ahmadawais/chartli)
  > CLI that turns plain numbers into terminal charts. ascii, spark, bars, columns, heatmap, unicode, br...
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [wozhenbang2004/AgentNexus](https://github.com/wozhenbang2004/AgentNexus)
  > Multi-Agent,MCP,RAG,SpringAI1.0.0,RE-ACT
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [okooo5km/memory-mcp-server](https://github.com/okooo5km/memory-mcp-server)
  > A Model Context Protocol server that provides knowledge graph management capabilities. 
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [phuryn/pm-skills](https://github.com/phuryn/pm-skills)
  > PM Skills Marketplace: 100+ agentic skills, commands, and plugins — from discovery to strategy, exec...
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [papercomputeco/stereOS](https://github.com/papercomputeco/stereOS)
  > A Linux based operating system hardened and purpose built for AI agents
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [MicrosoftDocs/Agent-Skills](https://github.com/MicrosoftDocs/Agent-Skills)
  > Curated Agent Skills for Microsoft & Azure – giving AI coding assistants structured, real-time exper...
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [johannesjo/parallel-code](https://github.com/johannesjo/parallel-code)
  > Run Claude Code, Codex, and Gemini side by side — each in      its own git worktree
- 🌟 👤 [gatieme](https://github.com/gatieme) Starred [cft0808/edict](https://github.com/cft0808/edict)
  > 🏛️ 三省六部制 · OpenClaw Multi-Agent Orchestration System — 9 specialized AI agents with real-time dashbo...

### [phodal](https://github.com/phodal)
- 🌟 👤 [phodal](https://github.com/phodal) Starred [thamaraiselvam/Routeweave](https://github.com/thamaraiselvam/Routeweave)
  > Scan once. Explore every endpoint, trace business logic, and see what breaks when your database chan...

### [ys1231](https://github.com/ys1231)
- 🌟 👤 [ys1231](https://github.com/ys1231) Starred [openclaw/clawhub](https://github.com/openclaw/clawhub)
  > Skill Directory for OpenClaw


---
*最后更新于 2026-03-10 04:06:23 UTC*
*历史记录保存在 `archive` 目录中。*
